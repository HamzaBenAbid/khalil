    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                double k =  (double) Math.pow(2,2);
                float f1 = (float) Math.pow((-2 * ((float) Math.log1p( (0.1 / a) / 3.7) + (5.74 /  Math.pow(re1, 0.9)))), -2);
                float f2 = (float) Math.pow((-2 * ((float) Math.log1p((0.1 / b) / 3.7) + (5.74 /  Math.pow(re2, 0.9)))), -2);

                float h1 = (float) ((f / a) * ((v1 * v1) / (2 * 9.81))) * f1;
                float h2 = (float) ((f / b) * ((v2 * v2) / (2 * 9.81))) * f2;
                float ress = h1 + h2;
                res.setText("s1 =/n" + s1 + "s2 =" + s2 + "v1 =" + v1 + "v2 =" + v2 + "w= " + w + "re1 =" + re1 + "re2 =" + re2 + "f1 = " + f1 + "f2 = " + f2 + "h1 = " + h1 + "h2 = " + h2 + "res = " + ress+"k = "+k);
            }
        });
